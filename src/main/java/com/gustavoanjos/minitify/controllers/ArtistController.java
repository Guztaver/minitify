...
public class ArtistController {
        ...
        @PostMapping
        public ResponseEntity<ArtistDTO.Response> createArtist(@RequestBody @Validated ArtistDTO artistDTO) {
                var artist = service.createArtist(artistDTO);
                return ResponseEntity.status(HttpStatus.CREATED).body(ArtistDTO.Response.fromArtist(artist));
        }
        ...
        @GetMapping("/all")
        public ResponseEntity<Collection<ArtistDTO.Response>> findAll(@RequestParam(required = false) String genre) {
                var artists = Optional.ofNullable(genre)
                                .filter(g -> !g.isEmpty())
                                .map(repository::findByGenre)
                                .orElseGet(repository::findAll)
                                .stream()
                                .map(ArtistDTO.Response::fromArtist)
        ...
        @PutMapping("/{id}")
        public ResponseEntity<ArtistDTO.Response> updateArtist(
                ...
                return ResponseEntity.ok(ArtistDTO.Response.fromArtist(updatedArtist));
        }
        ...
        @GetMapping("/{id}")
        public ResponseEntity<ArtistDTO.Response> findById(@PathVariable @Validated String id) {
                ...
                return ResponseEntity.ok(ArtistDTO.Response.fromArtist(artist));
        }
        ...
        public ResponseEntity<Set<Album>> getArtistAlbums(@PathVariable @Validated String id,
